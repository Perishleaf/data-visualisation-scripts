...
def make_country_table(countryName):
    ...
    if countryName == 'Australia':
    	countryTable = countryTable[['Province/State', 'Active', 'Confirmed', 'Recovered', 'Deaths', 'Death rate', 'Tests', 'Confirmed/100k', 'lat', 'lon']]
    else:
...
def make_europe_table(europe_list):
  ...
  europe_table = europe_table[['Country/Region', 'Active', 'Confirmed', 'Recovered', 'Deaths', 'Death rate', 'Tests', 'Confirmed/100k', 'lat', 'lon']]
...
def make_dcc_country_tab(countryName, dataframe):
    ...
    if countryName == 'Australia' or countryName == 'Europe':
      return dcc.Tab(
               ...
               children=[dash_table.DataTable(
                    ...
                    columns=[{"name": 'Province/State', "id": 'Province/State'}
                            ...
                            [{"name": i, "id": i, "type": "numeric","format": FormatTemplate.percentage(2)}
                             if i == 'Death rate' else {"name": i, "id": i, 'type': 'numeric', 'format': Format(group=',')}
                                for i in dataframe.columns[1:8]],
                    ...
                    style_header={'backgroundColor': '#ffffff',
                                  'fontWeight': 'bold'},
                    style_cell_conditional=[{'if': {'column_id': 'Province/State'}, 'width': '18%'},
                                            {'if': {'column_id': 'Country/Regions'}, 'width': '18%'},
                                            {'if': {'column_id': 'Active'}, 'width': '11%'},
                                            {'if': {'column_id': 'Confirmed'}, 'width': '12%'},
                                            {'if': {'column_id': 'Recovered'}, 'width': '12%'},
                                            {'if': {'column_id': 'Deaths'}, 'width': '11%'},
                                            {'if': {'column_id': 'Death rate'}, 'width': '12%'},
                                            {'if': {'column_id': 'Tests'}, 'width': '11%'},    
                                            {'if': {'column_id': 'Confirmed/100k'}, 'width': '13%'},
...
dfSum = dfSum[['Country/Region', 'Active',
    'Confirmed', 'Recovered', 'Deaths', 'Death rate', 'Tests', 'Confirmed/100k', 'lat', 'lon','Population']]
...
dfSum = dfSum.replace({'Tests': 0}, 'N/A')
                                          ...
                                          dcc.Tab(label='Worldwide',
                                              ...
                                              children=[
                                                  dash_table.DataTable(
                                                      ...
                                                      style_header={'backgroundColor': '#ffffff',
                                                                    'fontWeight': 'bold'},
                                                      style_cell_conditional=[{'if': {'column_id': 'Country/Regions'}, 'width': '18%'},
                                                                              {'if': {'column_id': 'Active'}, 'width': '11%'},
                                                                              {'if': {'column_id': 'Confirmed'}, 'width': '12%'},
                                                                              {'if': {'column_id': 'Recovered'}, 'width': '12%'},
                                                                              {'if': {'column_id': 'Deaths'}, 'width': '11%'},
                 ...
                 children=[
                     ...
                     html.P(style={'textAlign': 'center', 'margin': 'auto'},
                            children=['Keep calm and stay home | ',
                                      ...
                                      html.A('COVID-19 infographic in Australia', href='https://www.health.gov.au/sites/default/files/documents/2020/04/coronavirus-covid-19-at-a-glance-coronavirus-covid-19-at-a-glance-infographic_13.pdf', target='_blank'),
